...
collect_integrations() {
  ...
  # GH_TOKEN is always needed — deploy.yml uses it to clone the private config repo
  echo ""
  echo -e "  ${YELLOW}GitHub Personal Access Token (private config repo 배포 인증용):${NC}"
  echo -e "  ${DIM}권한: Contents: Read (Fine-grained) 또는 repo scope (classic)${NC}"
  ask_validated_secret "  GH_TOKEN" GH_TOKEN "ghp_|github_pat_" "ghp_... 또는 github_pat_..."
}
...
generate_env_example() {
  ...
  { echo "# Hermes Agent — Environment Variables"
    echo "# Generated by GitHub Actions deploy from GitHub Secrets"
    echo "# NEVER commit .env to git"
    echo ""
    echo "# ── Anthropic ──────────────────────────────────────────"
    echo "ANTHROPIC_API_KEY="
    echo ""
    echo "# ── GitHub (private repo deploy auth) ──────────────────"
    echo "GH_TOKEN="; } > "$f"
  [[ "$USE_SLACK" == "true" ]] && {
    printf '\n# ── Slack ──────────────────────────────────────────────\n'
...
generate_dot_env() {
  ...
  [[ "$USE_GITHUB_WEBHOOK" == "true" ]] && {
    echo "GITHUB_WEBHOOK_SECRET=${GITHUB_WEBHOOK_SECRET}"; } >> "$f"
  chmod 600 "$f"
}
...
register_github_secrets() {
  ...
  echo ""
  gh secret set ANTHROPIC_API_KEY --body "$ANTHROPIC_API_KEY" --repo "$repo" && ok "ANTHROPIC_API_KEY"
  gh secret set GH_TOKEN --body "$GH_TOKEN" --repo "$repo" && ok "GH_TOKEN"

  # Platform tokens
  if [[ "$USE_SLACK" == "true" ]]; then
    gh secret set SLACK_BOT_TOKEN --body "$SLACK_BOT_TOKEN" --repo "$repo" && ok "SLACK_BOT_TOKEN"
    gh secret set SLACK_APP_TOKEN --body "$SLACK_APP_TOKEN" --repo "$repo" && ok "SLACK_APP_TOKEN"
    [[ -n "$SLACK_ALLOWED_USERS" ]] && \
      gh secret set SLACK_ALLOWED_USERS --body "$SLACK_ALLOWED_USERS" --repo "$repo" && ok "SLACK_ALLOWED_USERS"
  fi

  if [[ "$USE_DISCORD" == "true" ]]; then
    gh secret set DISCORD_BOT_TOKEN --body "$DISCORD_BOT_TOKEN" --repo "$repo" && ok "DISCORD_BOT_TOKEN"
    gh secret set DISCORD_GUILD_ID --body "$DISCORD_GUILD_ID" --repo "$repo" && ok "DISCORD_GUILD_ID"
  fi

  if [[ "$USE_TELEGRAM" == "true" ]]; then
    gh secret set TELEGRAM_BOT_TOKEN --body "$TELEGRAM_BOT_TOKEN" --repo "$repo" && ok "TELEGRAM_BOT_TOKEN"
    [[ -n "$TELEGRAM_ALLOWED_USERS" ]] && \
      gh secret set TELEGRAM_ALLOWED_USERS --body "$TELEGRAM_ALLOWED_USERS" --repo "$repo" && ok "TELEGRAM_ALLOWED_USERS"
  fi

  if [[ "$USE_GITHUB_WEBHOOK" == "true" ]]; then
    # GitHub Actions secret names cannot start with GITHUB_ — store as WEBHOOK_SECRET
    gh secret set WEBHOOK_SECRET --body "$GITHUB_WEBHOOK_SECRET" --repo "$repo" && ok "WEBHOOK_SECRET"
  fi
}
...
print_next_steps() {
  ...
  echo ""
  echo -e "  1. 서버에서 config repo clone 및 실행:"
  echo -e "     ${BLUE}git clone https://github.com/${repo}.git /opt/${AGENT_NAME}-config${NC}"
  echo -e "     ${BLUE}cd /opt/${AGENT_NAME}-config && docker compose up -d${NC}"
  echo ""
  echo -e "  이후 ${BOLD}main으로 push할 때마다${NC} GitHub Actions가 자동으로:"
  echo -e "     • GitHub Secrets → 서버에 .env 재생성"
  echo -e "     • docker compose pull + restart"
  echo ""
  echo -e "  2. 상세 가이드: ${BOLD}${OUTPUT_DIR}/docs/server-setup.md${NC}"
  echo ""
}
